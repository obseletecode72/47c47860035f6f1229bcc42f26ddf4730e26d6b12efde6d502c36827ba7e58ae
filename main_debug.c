...
static int tornar_daemon(void) {
    pid_t pid;
    printf("[DEBUG] tornar_daemon: starting daemonization\n");
    pid = fork();
    if (pid < 0) {
        printf("[DEBUG] tornar_daemon: first fork failed: %s\n", strerror(errno));
        return -1;
    }
    if (pid > 0) {
        printf("[DEBUG] tornar_daemon: parent exiting after first fork, child pid=%d\n", pid);
        _exit(0);
    }
    printf("[DEBUG] tornar_daemon: first child running, pid=%d\n", getpid());
    if (setsid() < 0) {
        printf("[DEBUG] tornar_daemon: setsid failed: %s\n", strerror(errno));
        return -1;
    }
    printf("[DEBUG] tornar_daemon: setsid succeeded, new session leader\n");
    signal(SIGHUP, SIG_IGN);
    pid = fork();
    if (pid < 0) {
        printf("[DEBUG] tornar_daemon: second fork failed: %s\n", strerror(errno));
        return -1;
    }
    if (pid > 0) {
        printf("[DEBUG] tornar_daemon: intermediate process exiting after second fork, grandchild pid=%d\n", pid);
        _exit(0);
    }
    printf("[DEBUG] tornar_daemon: daemon process running, pid=%d\n", getpid());
    chdir("/");
    umask(0);
    printf("[DEBUG] tornar_daemon: closing all file descriptors and redirecting to /dev/null\n");
...
int main(int argc, char **argv) {
    ...
    printf("[DEBUG] main: started, pid=%d, argc=%d\n", getpid(), argc);
    for (int i = 0; i < argc; i++) {
        printf("[DEBUG] main: argv[%d]='%s'\n", i, argv[i]);
    }

    printf("[DEBUG] main: checking euid, euid=%d\n", geteuid());
    if (geteuid() != 0) {
        printf("[DEBUG] main: not root (euid=%d), exiting\n", geteuid());
        _exit(0);
    }
    printf("[DEBUG] main: running as root\n");

    ssize_t rlen = readlink("/proc/self/exe", g_self_path, sizeof(g_self_path) - 1);
    if (rlen > 0) {
        g_self_path[rlen] = 0;
        printf("[DEBUG] main: self path from /proc/self/exe: '%s'\n", g_self_path);
    } else {
        printf("[DEBUG] main: readlink /proc/self/exe failed: %s\n", strerror(errno));
        if (argc > 0 && argv[0][0] == '/') {
            strncpy(g_self_path, argv[0], sizeof(g_self_path) - 1);
            printf("[DEBUG] main: using argv[0] as self path: '%s'\n", g_self_path);
        } else {
            printf("[DEBUG] main: argv[0] is not absolute path, g_self_path remains empty\n");
        }
    }

    printf("[DEBUG] main: about to daemonize\n");
    if (tornar_daemon() < 0) {
        printf("[DEBUG] main: daemonization failed, exiting\n");
        _exit(1);
    }
